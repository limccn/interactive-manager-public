(function () {
  ...
  var readyLottery = function(){
    ...
    if(localStorage.getItem('lotteryWinnerList')) settings.winnerList = JSON.parse(localStorage.getItem('lotteryWinnerList'));
    initDom(settings.$el);
...
//    console.log("Lottery: Ignore user #",settings.winnerList);
    ...
    localStorage.setItem('lotteryWinnerList',JSON.stringify(settings.winnerList));
...
//      console.log('Lottery: moveToTarget #', currentTarget);
  ...
  var cleanHistory = function(){
    if (confirm('确认要删除历史记录?')==true){
      ...
      localStorage.setItem('lotteryWinnerList','');
