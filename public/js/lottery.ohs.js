(function () {
  ...
  var diceIconHtml = "<i class='dh-icon dh-icon-dice'>🎲</i>"
  var doneIconHtml = "<i class='dh-icon dh-icon-dice'>💾</i>"
  var saveIconHtml = "<i class='dh-icon dh-icon-dice'>🔨</i>"
  var okayIconHtml = "<i class='dh-icon dh-icon-okay'>👌</i>"
  ...
  var initDom = function(dom){
    ...
    $('.dh-number-inc').click(function () {
      if (settings.number < 10) {
        settings.number += 1;
        localStorage.setItem('lotteryConfigNumber', settings.number);
        $('.dh-number').text(settings.number);
      }
    });
    $('.dh-number').click(function () {
      settings.number = 1;
      localStorage.setItem('lotteryConfigNumber', settings.number);
      $('.dh-number').text(settings.number);
    })
    $('.dh-number-dec').click(function () {
  ...
  var readyLottery = function(){
    ...
    if(localStorage.getItem('lotteryHistory.ohs')) settings.winnerHistory = JSON.parse(localStorage.getItem('lotteryHistory.ohs'));
    if(localStorage.getItem('lotteryWinnerList.ohs')) settings.winnerList = JSON.parse(localStorage.getItem('lotteryWinnerList.ohs'));
    initDom(settings.$el);
...
//    console.log("Lottery: Ignore user #",settings.winnerList);
    ...
    localStorage.setItem('lotteryHistory.ohs',JSON.stringify(settings.winnerHistory));
    localStorage.setItem('lotteryWinnerList.ohs',JSON.stringify(settings.winnerList));
...
//      console.log('Lottery: moveToTarget #', currentTarget);
  ...
  var cleanHistory = function(){
    if (confirm('确认要删除历史记录?')==true){
      localStorage.setItem('lotteryHistory.ohs','');
      localStorage.setItem('lotteryWinnerList.ohs','');
