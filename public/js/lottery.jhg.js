(function () {
  ...
  var readyLottery = function(){
    ...
    if(localStorage.getItem('lotteryHistory.jhg')) settings.winnerHistory = JSON.parse(localStorage.getItem('lotteryHistory.jhg'));
    initDom(settings.$el);
...
//    console.log("Lottery: Ignore user #",settings.winnerList);
    ...
    localStorage.setItem('lotteryHistory.jhg',JSON.stringify(settings.winnerHistory));
...
//      console.log('Lottery: moveToTarget #', currentTarget);
  ...
  var cleanHistory = function(){
    if (confirm('确认要删除历史记录?')==true){
      localStorage.setItem('lotteryHistory.jhg','');
